...
$r->get('/post/{id}', function($vars=[]){
    // echo "라우트 포스트 실행<br>";
    return controller("PostController", "post", $vars);
});

$r->get('/post/{year}/{month}/{day}/{name}', function($vars=[]){
    echo "라우트 포스트 실행<br>";
    print_r($vars);
    return controller("PostController", "_view", $vars);
});


$r->get('/admin/members', function($vars=[]){
    return controller("Admin\MembersController", "index", $vars);
});
